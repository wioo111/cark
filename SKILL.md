...
description: >
  ...
  配个论文翻译工具 / 把这篇 PDF 论文转成飞书双语文档 / 部署 Paper2Lark / 装 MinerU 转飞书"。
---
...
确认产物质量——别只靠肉眼，跑这组确定性自检（把 `<bilingual.md>` 换成实际双语产物路径）：

```bash
PY=.venv/Scripts/python.exe; [ -f "$PY" ] || PY=.venv/bin/python
BL="<bilingual.md>"   # 形如 runtime/output/<stem>/auto/<stem>_linearized_bilingual.md
test -f "$BL" && echo "✓ 双语产物存在" || echo "✗ 双语产物缺失 → 解析/翻译没跑完"
echo "公式标记($)数: $(grep -o '\$' "$BL" | wc -l)   # >0 说明公式保留了"
echo "总行数: $(wc -l < "$BL")"
# 中英对照粗检:既有中文(CJK)又有英文行，说明是双语而非单语
"$PY" -c "import re,sys; t=open('$BL',encoding='utf-8').read(); cjk=len(re.findall(r'[一-鿿]',t)); en=len(re.findall(r'[A-Za-z]{4,}',t)); print(f'中文字符 {cjk}，英文词 {en} →', '✓ 双语' if cjk>50 and en>50 else '✗ 疑似单语，检查翻译是否生效')"
```

数字都正常（双语产物存在、`$`>0、中英都够多）后，去掉 `--prepare-only` 真正推送飞书
...
- 凭据已写入环境变量，以后新终端直接 `pdf_to_feishu_docx.py --input-pdf ... --translate`
  （加不加 `--backend cloud` 取决于用哪种后端）。
- 可调环境变量与「稳定性与调优」细节**不在此重复**，直接指给用户看 README 的对应章节
  （云端轮询超时 `MINERU_CLI_TIMEOUT`、本地 `MINERU_GPU_IDLE_MIB` / `MINERU_WMI_SHIM` /
  `TRANSLATE_FAIL_RATIO_LIMIT` / `MINERU_SKIP_PREFLIGHT` 等都在那里）。
