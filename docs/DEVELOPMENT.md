...
解析（MinerU）→ 阅读顺序线性化 → 双语翻译（任意 OpenAI 兼容 LLM）→ 推送飞书 docx，保留公式/图/表。
...
### 4. 环境事实（本机）
- 飞书 + 翻译 API 凭据已存入 Windows 用户级环境变量，新终端自动读（本机翻译恰用 DeepSeek，
  但工具不绑定服务商，任意 OpenAI 兼容 LLM 均可——见 SKILL「凭据获取引导」）。
- WMI 服务已恢复正常（2026-06-08 复测 `check_wmi` 返回 OK；shim 在 auto 模式下探测到健康会自动不打补丁）。
