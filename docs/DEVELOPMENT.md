...
1. **写 `SKILL.md`**：Skill Download 范式的核心载体。分阶段（环境探测→装 MinerU→应用补丁→
   下模型→配凭据→体检跑通），供 AI 喂入后自然语言交互式安装。README 已留占位说明。
2. **手动安装 fallback 文档**：为没有 agentic AI 的用户补一份纯手动步骤（部分已在 README）。
3. 首个 git commit（本次已 `git init` 并整理好暂存区，commit message 建议：
   `init: paper2lark — PDF 论文转飞书双语文档的一键编排工具`）。
